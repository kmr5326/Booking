...
public class MemberControllerTest extends ControllerTest {
    ...
    void t2() throws Exception{
        String loginId= "1234";
        MemberInfoResponseDto responseDto=new MemberInfoResponseDto("1234","email",10,"MALE","mono","monono","addr","img","google");

        when(memberService.loadMemberInfo(any())).thenReturn(responseDto);

        mockMvc.perform(get(baseUrl + "/memberInfo/{loginId}", loginId)
                        .contentType(MediaType.APPLICATION_JSON) // JSON
        ).andExpect(status().isOk()) // 200 반환
                .andDo(
                        document("/members/info",
                                preprocessResponse(prettyPrint()),
                                pathParameters(
